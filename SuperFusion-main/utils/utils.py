...
def randflow(img,angle=7,trans=0.07,ratio=1,sigma=15,base=500):
    ...
    sigma_tensor = (sigma, sigma)
    kernel_size = ((3*sigma)//2*2+1,(3*sigma)//2*2+1)
    for i in range(5):
        disp = KF.gaussian_blur2d(disp,kernel_size=kernel_size,sigma=sigma_tensor)
    disp = KF.gaussian_blur2d(disp,kernel_size=kernel_size,sigma=sigma_tensor).permute(0,2,3,1)*ratio
...
# img = Image.open("/home/yuan/桌面/20230218image_fusion_paper8/SuperFusion-main/dataset/test/MSRS/ir/00081D.png")
