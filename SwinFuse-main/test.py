...
def run_demo(model, infrared_path, visible_path, output_path_root, index, f_type):
    ...
    for img_fusion in img_fusion_list:
        #file_name = 'fusion' + '_' + str(index) + '_swinfuse_' + str(output_count) + '_' + 'f_type' + '.bmp'
...
def main():
    # run demo
    test_path = "/home/yuan3080/桌面/Imagefusion_paper7/"
    ...
    with torch.no_grad():
        ...
        # for i in range(1000, 1040):
            index = i + 1
            infrared_path = test_path + 'part321_ir/' + str(index) + '.bmp'
            visible_path = test_path + 'part321_rgb/' + str(index) + '.bmp'
